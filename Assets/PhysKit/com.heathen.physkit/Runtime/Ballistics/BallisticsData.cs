...
namespace HeathenEngineering.PhysKit
{
    ...
    public struct BallisticsData
    {
        ...
        [Obsolete("Please use Predict instead")]
        public BallisticPath Perdict(Vector3 from, Collider fromCollider, float resolution, float distanceLimit, LayerMask collisionLayers) => Predict(from, fromCollider, resolution, distanceLimit, collisionLayers);
        ...
        /// <returns></returns>
        public BallisticPath Predict(Vector3 from, Collider fromCollider, float resolution, float distanceLimit, LayerMask collisionLayers, Vector3 gravity)
        {
            if (velocity == Vector3.zero)
                return new BallisticPath
                {
            ...
            if (gravity == Vector3.zero)
                ...
                impact = API.Ballistics.SphereCast(from, fromCollider, velocity, gravity, radius, resolution, distanceLimit, collisionLayers, out hit, out path, out flightDistance);
