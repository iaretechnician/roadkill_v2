...
namespace HeathenEngineering.PhysKit
{
    public class TrickShot : MonoBehaviour
    {
        ...
        public void Predict()
        {
            ...
            {
                prediction.Clear();
                prediction.Add(projectileSettings.Predict(selfTransform.position, null, resolution, distance, collisionLayers));
            }
            else if (distanceIsTotalLength)
            {
                prediction = new List<BallisticPath>();
                var current = projectileSettings.Predict(selfTransform.position, null, resolution, distance, collisionLayers);
                    ...
                    && bounces > 0)
                {
                    ...
                    {
                        current = project.Predict(position, current.impact.Value.collider, resolution, remainingDistance, collisionLayers);
                        prediction.Add(current);
            ...
            else
            {
                    ...
                    && bounces > 0)
                {
                    ...
                    for (int i = 0; i < bounces; i++)
                    {
                        current = project.Predict(position, current.impact.Value.collider, resolution, distance, collisionLayers);
                        prediction.Add(current);
