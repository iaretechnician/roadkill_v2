...
namespace HeathenEngineering.DEMO
{
    ...
    public class Scene11MirrorPlayerController : NetworkBehaviour
    {
        public override void OnStartAuthority()
        {
            ...
            if (ulong.TryParse(Mirror.NetworkManager.singleton.networkAddress, out var id))
            {
                Authentication.GetAuthSessionTicket(new Steamworks.CSteamID(id), (result, ioError) =>
                {
                    if (!ioError)
